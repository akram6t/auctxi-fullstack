...
const assignedAuctions = [
  { id: 'AUC-101', name: 'IPL Mega Auction 2024', date: 'Oct 20, 2024', players: 350, teams: 10, status: 'Live' },
  { id: 'AUC-102', name: 'WPL Mini Auction', date: 'Nov 15, 2024', players: 120, teams: 5, status: 'Upcoming' },
  { id: 'AUC-103', name: 'Domestic T20 League', date: 'Dec 05, 2024', players: 200, teams: 8, status: 'Drafting' },
];

const AssignedAuctionsTable = ({ onEnterLive }) => {
  return (
          ...
          <tbody className="bg-white dark:bg-secondary-900 divide-y divide-secondary-200 dark:divide-secondary-800">
            {assignedAuctions.map((auction) => (
              <tr key={auction.id} className="hover:bg-secondary-50 dark:hover:bg-secondary-800/50 transition-colors">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm text-secondary-600 dark:text-secondary-300">
                  {auction.players} Players / {auction.teams} Teams
                </td>
                <td className="px-6 py-4 whitespace-nowrap">
                ...
                <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                  {auction.status === 'Live' ? (
                    ...
                    >
                      <IconPlayerPlay size={16} className="mr-1" /> Enter Console
                    </button>
                  ) : (
